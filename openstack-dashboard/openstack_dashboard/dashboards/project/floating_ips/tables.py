...
class ForwardingPort(tables.LinkAction):
    name = "forwarding"
    verbose_name = _("Forwarding Port To Instance")
    classes = ("ajax-modal",)
    icon = "link"
    url = "horizon:project:floating_ips:forwarding"
...
#                    quota_exceeded=_("(Quota exceeded)"))
#        else:
        self.verbose_name = _("Forwarding Port To Instance")
        classes = [c for c in self.classes if c != "disabled"]
        self.classes = classes
...
class DeletePortForwarding(tables.LinkAction):
    name = "delete_port_forwarding"
    verbose_name = _("Delete Port Forwarding")
    url = "horizon:project:floating_ips:delete_port_forwarding"
    ...
    def get_link_url(self, datum):
        return reverse("horizon:project:floating_ips:delete_port_forwarding",
                       kwargs={'ip_id': datum.id})
...
class FloatingIPsTable(tables.DataTable):
    ...
    class Meta(object):
        ...
        table_actions = (AllocateIP, ForwardingPort, ReleaseIPs, FloatingIPsFilterAction)
        row_actions = (AssociateIP, DeletePortForwarding,  DisassociateIP, ReleaseIPs)
